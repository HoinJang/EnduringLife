...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//
CFBXMesh::CFBXMesh(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList) : CMeshIlluminated(pd3dDevice, pd3dCommandList)
{
}

CFBXMesh::CFBXMesh(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, UINT nVertices, XMFLOAT3 *pxmf3Positions, XMFLOAT3 *pxmf3Normals, XMFLOAT2 *pxmf2UVs, UINT nIndices, UINT *pnIndices, XMFLOAT4 *pxmf4BoneWeight, XMFLOAT4 *pxmf4BoneIndices) : CMeshIlluminated(pd3dDevice, pd3dCommandList)
{
	m_nStride = sizeof(CFBXVertex);
	m_nVertices = nVertices;

	CFBXVertex *pVertices = new CFBXVertex[m_nVertices];
	for (UINT i = 0; i < m_nVertices; i++)
	{
		pVertices[i] = CFBXVertex(pxmf3Positions[i], pxmf3Normals[i], pxmf2UVs[i], pxmf4BoneWeight[i], pxmf4BoneIndices[i]);
	}
	m_pd3dVertexBuffer = ::CreateBufferResource(pd3dDevice, pd3dCommandList, pVertices, m_nStride * m_nVertices, D3D12_HEAP_TYPE_DEFAULT, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER, &m_pd3dVertexUploadBuffer);
	m_d3dVertexBufferView.BufferLocation = m_pd3dVertexBuffer->GetGPUVirtualAddress();
	m_d3dVertexBufferView.StrideInBytes = m_nStride;
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
CMeshIlluminatedTexturedNormalMapped::CMeshIlluminatedTexturedNormalMapped(ID3D12Device * pd3dDevice, ID3D12GraphicsCommandList * pd3dCommandList) :CMeshIlluminatedTextured(pd3dDevice, pd3dCommandList)
{
}

CMeshIlluminatedTexturedNormalMapped::CMeshIlluminatedTexturedNormalMapped(ID3D12Device * pd3dDevice, ID3D12GraphicsCommandList * pd3dCommandList, UINT nVertices, XMFLOAT3 * pxmf3Positions, XMFLOAT3 * pxmf3Normals, XMFLOAT2 * pxmf2UVs, XMFLOAT3 * pxmf3Tangents, UINT nIndices, UINT * pnIndices) : CMeshIlluminatedTextured(pd3dDevice, pd3dCommandList)
{ 
	//tangent는 받지 말고 쉐이더에서 계산하도록...
	//이 생각이 틀렸던 거야....
	//탄젠트- 모델이 갖고 있는 정보임.
	
	m_nStride = sizeof(CIlluminatedTexturedNormalMappedVertex);
	m_nVertices = nVertices;

	CIlluminatedTexturedNormalMappedVertex *pVertices = new CIlluminatedTexturedNormalMappedVertex[m_nVertices];
	for (UINT i = 0; i < m_nVertices; i++) pVertices[i] = CIlluminatedTexturedNormalMappedVertex(pxmf3Positions[i], pxmf3Normals[i], pxmf2UVs[i],pxmf3Tangents[i]);
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
CPlaneMeshTextured::CPlaneMeshTextured(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, float fWidth, float fHeight, float fDepth) : CMeshTextured(pd3dDevice, pd3dCommandList)
{
	m_nVertices = 6;
	m_nStride = sizeof(CTexturedVertex);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
CWaterMeshTextured::CWaterMeshTextured(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, float fWidth, float fHeight, float fDepth) : CMeshTextured(pd3dDevice, pd3dCommandList)
{
	m_nVertices = 6;
	m_nStride = sizeof(C2TexturedVertex);
	m_d3dPrimitiveTopology = D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST;

	C2TexturedVertex pVertices[6];

	float fx = (fWidth * 0.5f), fy = (fHeight * 0.5f), fz = (fDepth * 0.5f);

	pVertices[0] = C2TexturedVertex(XMFLOAT3(+fx, fy, +fz), XMFLOAT2(1.0f, 0.0f), XMFLOAT2(1.0f, 0.0f));
	pVertices[1] = C2TexturedVertex(XMFLOAT3(+fx, fy, -fz), XMFLOAT2(1.0f, 1.0f), XMFLOAT2(1.0f, 1.0f));
	pVertices[2] = C2TexturedVertex(XMFLOAT3(-fx, fy, +fz), XMFLOAT2(0.0f, 1.0f), XMFLOAT2(0.0f, 1.0f));
	pVertices[3] = C2TexturedVertex(XMFLOAT3(-fx, fy, -fz), XMFLOAT2(0.0f, 1.0f), XMFLOAT2(0.0f, 1.0f));
	pVertices[4] = C2TexturedVertex(XMFLOAT3(-fx, fy, +fz), XMFLOAT2(0.0f, 0.0f), XMFLOAT2(0.0f, 0.0f));
	pVertices[5] = C2TexturedVertex(XMFLOAT3(+fx, fy, -fz), XMFLOAT2(1.0f, 0.0f), XMFLOAT2(1.0f, 0.0f));
...
CWaterMeshTextured::~CWaterMeshTextured()
{
}

/////////////////////////////////////////////////////////////////
CCubeMeshTextured::CCubeMeshTextured(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, float fWidth, float fHeight, float fDepth) : CMeshTextured(pd3dDevice, pd3dCommandList)
{
	m_nVertices = 36;
	m_nStride = sizeof(CTexturedVertex);
	m_nOffset = 0;
...
XMFLOAT4 CHeightMapGridMesh::OnGetColor(int x, int z, void *pContext)
{
	...
	CHeightMapImage *pHeightMapImage = (CHeightMapImage *)pContext;
	XMFLOAT3 xmf3Scale = pHeightMapImage->GetScale();
	XMFLOAT4 xmf4IncidentLightColor(0.34f, 0.34f, 0.34f, 1.0f);
...
CBillBoardMesh::CBillBoardMesh(ID3D12Device * pd3dDevice, ID3D12GraphicsCommandList * pd3dCommandList, float fWidth, float fHeight, float fDepth)
