...
{
	// 한 캐릭터 당 최대 뼈대 개수
    matrix gBoneTransform[64] : packoffset(c0);
};
//HP(MP,EXP,IDX)정보
...
cbuffer cbSkillInfo : register(b9)
{
    float gMaxIdx;
    float m_SkillNum;
}
...
float4 PSTexturedLighting(VS_TEXTURED_LIGHTING_OUTPUT input) : SV_TARGET
{
    ...
    float4 cResult = (lerp(cColor, cIllumination, 0.2f));
    return (Fog(cResult, input.positionW));
}

float4 PSFBX(VS_TEXTURED_LIGHTING_OUTPUT input) : SV_TARGET
{
    float4 cColor = gtxtTexture.Sample(gWrapSamplerState, input.uv);
    if ((cColor.r < 0.1f) && (cColor.g < 0.1f) &&  (cColor.b < 0.1f))    discard;

    input.normalW = normalize(input.normalW);
    float4 cIllumination = Lighting(input.positionW, input.normalW);

 //   return (lerp(cColor, cIllumination, 0.1f));
    float4 cResult = (lerp(cColor, cIllumination, 0.1f));
    return (Fog(cResult, input.positionW));
}
...
float4 PSSkyBox(VS_TEXTURED_OUTPUT input) : SV_TARGET
{
    ...
    return (SkyFog(cColor, input.position.xyz));
...
float4 PSBillboardTextured(VS_TEXTURED_OUTPUT input) : SV_TARGET
{
    ...
    if ((cColor.r < 0.2) && (cColor.g < 0.2) && (cColor.b < 0.2))
        discard;
    else if ((cColor.r > 0.9) && (cColor.g > 0.9) && (cColor.b > 0.9))
        discard;
...
float4 PSSkinned(VS_SKINNED_OUTPUT input) : SV_Target
{
    input.normalW = normalize(input.normalW);
    float4 cIllumination = Lighting(input.positionW, input.normalW);
    float4 cColor = gtxtTexture.Sample(gWrapSamplerState, input.texCoord); 
...
VS_TEXTURED_OUTPUT VSHPBar(VS_TEXTURED_INPUT input)
{
    ...
    output.position = float4(input.position.x * gHPratio - (0.25f * (1 - gHPratio)), input.position.y - 0.8f, input.position.z, 1.f);
...
VS_TEXTURED_OUTPUT VSMPBar(VS_TEXTURED_INPUT input)
{
    VS_TEXTURED_OUTPUT output;

    //output.position = float4(input.position.x * gHPratio + (0.5f * gHPratio) - 0.5f, input.position.y - 0.85f, input.position.z, 1.f);
    output.position = float4(input.position.x * gHPratio - (0.25f * (1 - gHPratio)), input.position.y - 0.85f, input.position.z, 1.f);
    output.uv = input.uv;
    return (output);
}

float4 PSMPBar(VS_TEXTURED_OUTPUT input) : SV_TARGET
{
    float4 cColor = gtxtBillboard.Sample(gWrapSamplerState, input.uv);
...
VS_TEXTURED_OUTPUT VSEXPBar(VS_TEXTURED_INPUT input)
{
    VS_TEXTURED_OUTPUT output;

    output.position = float4(input.position.x * gHPratio + (gHPratio) - 1.f, input.position.y - 0.985f, input.position.z + 0.15f, 1.0f);
    output.uv = input.uv;
    return (output);
}

float4 PSEXPBar(VS_TEXTURED_OUTPUT input) : SV_TARGET
{
    float4 cColor = gtxtBillboard.Sample(gWrapSamplerState, input.uv);
...
////////////////////////////////////////////////////////////////////
Texture2D gtxtSkillBillboard[4]: register(t11);

VS_TEXTURED_OUTPUT VSEffect(VS_TEXTURED_INPUT input)
{
    VS_TEXTURED_OUTPUT output;
    output.position = mul(mul(mul(float4(input.position, 1.0f), gmtxGameObject), gmtxView), gmtxProjection);
    output.uv.x = (gHPratio / gMaxIdx) + input.uv.x * (1.0 / gMaxIdx);
    output.uv.y = input.uv.y; //ㅠㅠㅠ왜 float 만 되는 거닝....ㅠㅠㅠ
...
{
    float4 cColor = gtxtSkillBillboard[m_SkillNum].Sample(gWrapSamplerState, input.uv);
...
{
    VS_TEXTURED_OUTPUT output;
    output.position = float4(input.position.x + gfXpos, input.position.y + gfYpos, input.position.z + gfZpos, 1.0f);

    if (gfMaxIdx == -1 || gHPratio == -1)
    {
        output.uv = input.uv;
    }
    else
    {
        output.uv.x = (gHPratio / gfMaxIdx) + input.uv.x * (1 / gfMaxIdx);
...
{
    float4 cColor = gtxtBillboard.Sample(gClampSamplerState, input.uv);
    if (cColor.r > 0.80f && cColor.b > 0.80f && cColor.g < 0.15f)  //마젠타 지우깅
        discard;
...
////////////////////////////////////////////////////////////////////
VS_TEXTURED_OUTPUT VSMonHPBar(VS_TEXTURED_INPUT input)
{
    VS_TEXTURED_OUTPUT output;
    output.position = mul(mul(mul(float4(input.position, 1.0f), gmtxGameObject), gmtxView), gmtxProjection);
    ...
    output.uv.x = (gHPratio + input.uv.x) * 0.5f;
    output.uv.y = input.uv.y;
...
float4 PSMonHPBar(VS_TEXTURED_OUTPUT input) : SV_TARGET
{
    float4 cColor = gtxtBillboard.Sample(gClampSamplerState, input.uv);
    if (cColor.r < 0.1f && cColor.b < 0.1f && cColor.g < 0.1f)    discard; //검정 지우기
    return (cColor);
}
