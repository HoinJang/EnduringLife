...
float4 Fog(float4 cColor, float3 vPosition)
{
    ...
    // FogParameter.x(Fog타입), FogParameter.z( Fog범위), FogParameter.z(원거리 Fog범위)
    ...
    // FogFactor에 따라 안개색상을 넣어준다. 
    float4 cColorByFog = lerp(cColor, float4(0.75f, 0.75f, 0.75f, 0.85f), fFogFactor);
    return (cColorByFog);
}

float4 SkyFog(float4 cColor, float3 vPosition)
{ //월드 좌표계서의 카메라좌표, 점의 좌표
    ...
    if (gvFogParameter.x == LINEAR_FOG)
    {
        float fFogRange = gvFogParameter.z - gvFogParameter.y;
        fFogFactor = saturate((fDistanceToCamera - gvFogParameter.y) / fFogRange);
        if (gvFogParameter.y <= 1000)
            fFogFactor += 0.9f;
    }

    float4 cColorByFog = lerp(cColor, float4(0.75f, 0.75f, 0.75f, 0.45f), fFogFactor);
    return (cColorByFog);
}